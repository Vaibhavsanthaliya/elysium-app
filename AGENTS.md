...
| EA-27 | Modal Obsidian Temple pass. CSS-only, `.modal`-scoped: removed iOS blue/pink from edit-task, past-day, and task-info modals; handle pill uses `--separator-warm`; title weight 500 + tighter tracking; labels JetBrains Mono bronze; underline-only inputs with bronze focus; dropdown arrow bronze; dark-mode save button bronze bg / near-black text; delete button desaturated rust; secondary actions use `--muted-text`. No HTML, `app.js`, state, or storage changes. (commit `b0cd6b1`) |
