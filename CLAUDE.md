...
| EA-177 | Body Desk Relief Lookup — replaced paced somatic step-through with a chip-based area lookup (Neck / Shoulders / Back / Wrists / Eyes); selecting a chip reveals one curated reset instruction; Returned calls existing `arriveBody()` unchanged; no new state; horizon marks retained; §19D constitution amendment; SW → `elysium-v84`. |
