...
| EA-34 | Stars/Progress deeper refinement. `js/render/progress.js`: `renderConstellationLines()` — algorithm changed from chain (consecutive-pair only) to all-pairs nearest-neighbor (every pair checked); maxDist changed from `cellUnit * 2.5` → `cellUnit * 1.9` (allows H/V/diagonal immediate neighbors, blocks skip-one long jumps — 1.15× adjacent ✓, 1.62× diagonal ✓, 2.30× skip-one ✗). `css/progress.css`: EA-34 block appended — `#pane-progress .milestone-badge` → `background: transparent`, `padding: 0`, `border-radius: 0`, JetBrains Mono 9px `var(--bronze)` (removes green/orange/indigo pills, renders as mono meta text); `.milestone-badge.active` → `var(--muted-text)`; `.milestone-badge.updated` → `display: none`; `.cal-day.logged` → stronger double-layer glow `0 0 8px rgba(201,165,107,0.55), 0 0 2px rgba(201,165,107,0.35)`; `.cal-day.today.logged` → preserves inset ring with stronger outer glow; `.cal-day.partial` → `rgba(201,165,107,0.22)` bg / `0.52` border / `0.38` color numeral (closer to prototype s2 state); `.constellation-svg line` opacity `0.35 → 0.42`. No HTML, state keys, task IDs, cycle logic, milestone thresholds, Supabase schema, IndexedDB, or photo logic changed. |
...
| EA-35 | Ritual philosophy pass. Part 1 — Temple domain vocabulary: updated all six domain card subtitles from generic noun phrases to their interaction-primitive verbs in Spectral italic (`remember`, `witness`, `arrive`, `hold`, `notice`, `close`); added `.temple-domain-archetype` mono micro-label for archetype names (Mnemosyne, Apollo, Atlas, Athena, Poseidon, Hypnos) as atmosphere only. Part 2 — Chronicle → Drift: rebuilt Chronicle from write-first to reading-first; `#pane-chronicle` now has `#chronicle-drift` (JS-rendered past entries above) and `.chronicle-today` wrapper (writing surface below, secondary); `js/render/chronicle.js` extended — added `renderDrift()` reading `state.chronicle.notes`, excluding today, sorted descending, with optional "a year ago today" bronze hairline surfacing and HTML escaping; `css/chronicle.css` EA-35 block appended — drift entries (Spectral italic body, mono bronze date, warm separator rows), year-ago highlight, empty-state italic placeholder; `css/temple.css` `.temple-domain-archetype` rule added (mono 8px, 0.16em tracking, muted 0.55 opacity). All existing IDs preserved (`#chronicle-textarea`, `#chronicle-status`, `#chronicle-save-btn`, `#chronicle-prompt-q`, `#temple-chronicle-state`). No state keys, task IDs, cycle logic, milestone thresholds, Supabase schema, IndexedDB, service worker, or DEFAULT_DATA changed. |
| EA-37 | Global header restraint pass. `index.html`: demoted the sticky shell header from an iOS-style Large Title block to a compact daily-status rail; removed the visible global `Today` h1 and flame icon; preserved `#date-label`, `#streak-pill`, and `#streak-num` IDs so existing `renderHeader()` and streak updates continue unchanged. `css/layout.css`: reduced header padding and glass intensity, centered the rail, muted the date meta, converted `.streak-pill` from a filled bronze badge/button into quiet JetBrains Mono meta text with only the number in bronze, and removed press scaling. `css/components.css`: removed the now-unused `.greeting` selector from the shared heading typography override. No JS, state keys, storage, sync, Supabase schema, IndexedDB, task IDs, cycle logic, milestone logic, service worker, manifest, or dependencies changed. |
| EA-39 | Progress Photos section heading alignment. `index.html`: replaced the Progress Weekly Photos old `.section-header` + circular `＋` add affordance with the existing `.section-hd` pattern — Spectral italic "Weekly photos" left label and mono bronze `Add photo` action at right, preserving `#add-photo-btn` and `#weekly-photo-input`. `css/progress.css`: added scoped `#pane-progress .photo-add-action` text-button styling with transparent background, bronze text, active opacity, and focus-visible outline. `js/services/photos.js`: copy-only empty-state update from "tap +" to "Use Add Photo". No photo logic, IndexedDB, state, storage, sync, Supabase schema, progress/calendar/constellation logic, service worker, manifest, or dependencies changed. |
| EA-41 | Settings page heading alignment. `index.html`: added a Settings page heading above the Account section with mono eyebrow `ACCOUNT & RITUAL` and Spectral italic `Settings`, matching the Obsidian Temple heading pattern used by Cycle, Chronicle, and Stars. `css/settings.css`: added scoped `#pane-settings` heading styles for JetBrains Mono eyebrow, DM Sans title, and Spectral italic bronze accent. No JS, state, settings behavior, auth behavior, storage, sync, Supabase schema, IndexedDB, service worker, manifest, or dependency changes. |
| EA-43 | Sleep bedtime domain, modal-first. Added minimal `sleep: { entries: {} }` state with migration validation for `YYYY-MM-DD` / `HH:MM` bedtime entries; added `js/domains/sleep.js`; activated the Sleep Temple domain card as a supporting ritual with last-bedtime/Quiet state; added `#sleep-modal` for “Close the day” bedtime entry and quiet 7-night history; wired save through existing localStorage/Supabase JSONB flow with Sleep entry union merge on sign-in; added scoped `#sleep-modal` CSS; updated service worker cache to `elysium-v4` and cached the Sleep module. No new tab, no dependencies, no Supabase schema change, no IndexedDB change, no Care task IDs, no 3-day cycle change, and no milestone threshold change. Ticket creation skipped for later batch Jira creation. |
